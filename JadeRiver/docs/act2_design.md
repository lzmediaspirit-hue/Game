# Act II · The Azure Expanse (v1.1) — design

The build prompt gives Act II in outline (zone tier 2, levels 55–81, ceiling Sage Sovereign 3,
Storm Ward attunement 10 → 60, twelve monsters, two bosses and a list of systems). This page fills
in the geography, the story and the order in which the systems unlock, so each build phase adds a
playable, tested slice. Names are original to Jade River.

## Premise

Past the Ascension Gate the valley's sky opens onto the Azure Expanse: storm plains, frozen heights
and a mirror lake under floating peaks, held together by the **Nine Peaks Alliance**, a league of
nine sects that taxes every sky road. Independent cultivators and the old clans live in its margins.

Three threads carry the act:

1. **The Grey Pilgrim.** A robed stranger who never casts a shadow is buying Hollow shards across the
   Expanse. He serves the man from the Cleansing vision, Yan Heng. He wants the seal the Tomb King of
   Sunscar still holds.
2. **Elder Gu.** The smuggler who fled with his ledger (Act I) has sold it to Starsea pirates, and
   the ledger names every valley family that ever paid him. He resurfaces at the Skyport Wreck.
3. **Lu's crossing.** Lu came through the Expanse long ago. Five journal pages mark his road; the last
   shows that he, too, once stood before the Tomb King.

## Regions and rooms (about 45)

| # | Region | Levels | Storm Ward | Rooms | Monsters and bosses |
|---|---|---|---|---|---|
| 1 | Cloudgate Port (town, teleport stone) | — | 0 | Arrival Terrace · Port Market · Skydock · Wayfarers' Inn | — |
| 2 | Thunderhorn Plains | 64–69 | 10 | Stormgrass Verge · Thunderhorn Flats · Lightning Scar · Herders' Camp (rest) | Spark Weasel, Thunderhorn Rhino |
| 3 | Rimefrost Heights | 67–72 | 20 | Frostpine Climb · Snow Ape Ledges · Rimefrost Summit · Hermit's Ice Cave (secret) | Frost Lynx, Snow Ape |
| 4 | Mirrorwater Lake | 68–75 | 25 | Reedless Shore · Mirror Shallows · Sentinel Causeway · Lake Shrine (insight) · Toad's Hollow (field boss) | Azure Carp Dragonet, River Sentinel, Thousand-Eye Toad (field, 68) |
| 5 | Nine Peaks (alliance seat, town) | — | 0 | Alliance Gate · Hall of Nine · Presence Terrace · Auction Pavilion · Trial Hall | — |
| 6 | Gale Canyons | 73–78 | 40 | Canyon Mouth · Kite Winds · Harpy Roosts · Windbridge | Wind Kite, Canyon Harpy |
| 7 | Ironroot Clan Hold (town) | — | 0 | Hold Gate · Clan Hearth · Ancestor Hall | — |
| 8 | Sunscar Desert | 73–81 | 50 | Glass Dunes · Scorpion Flats · Worm Sea · Oasis of Bones (rest) | Sandstorm Scorpion, Dune Worm |
| 9 | Tomb of Sunscar (dungeon) | 77 | 55 | Sealed Gate · Hall of Sand Kings · Mirror Crypt · Throne of the Tomb King | Tomb King of Sunscar (77) |
| 10 | Skyport Wreck (Starsea shore) | 76–81 | 60 | Broken Pier · Pirate Deck · Riven Peak · Starsea Launch | Starsea Pirate, rogue Nine Peaks Disciple |

## Realms and systems by stage (Part 4 table)

| Stage | Unlocks |
|---|---|
| Heaven Glimpse 3 | The crossing; currency exchange; zone ceiling lifts in the Expanse; Storm Ward jades (from the free broker, so the plains can be walked before Sage) |
| Sage 1 | Sage Qi (×1.7), Spirit-grade refining, alliance or independent path, NPC auction house (account slot 10) |
| Sage 2 | Clans (Ironroot adoption quest) |
| Sage 3 | Starsea survival, star charts, vessels; Sage 3 → Sage Sovereign 1 needs a full Sage Qi reserve and one Dao at Adaptation (tier 5) |
| Sage Sovereign 1 | Sovereign Settling Pill, sect token upgrade, Awakened pets |
| Sage Sovereign 2 | Sect war event |
| Sage Sovereign 3 | Presence Trial (the Will Manifest breakthrough event), Starsea travel to the Lantern Star Field (Act III, planned) |

## Main story (chapters 11–16)

| Chapter | Quests |
|---|---|
| 11 Beyond the Gate (HG3–Sage 1) | Through the Gate (arrive at Cloudgate Port) · A Sky Full of Toll Roads (meet the Alliance factor and the free broker) · Storm in the Blood (Storm Ward jades on the plains) · Horns for the Furnace (thunderhorn horns for the Sage pill) · Sage (break through at the port's condensing hall) |
| 12 The Grey Pilgrim (Sage 1–2) | Shards for Sale (follow the buyer across the plains) · Frost and Silence (the hermit on Rimefrost) · The Mirror Remembers (Lake Shrine vision; Thousand-Eye Toad) |
| 13 The Nine Peaks (Sage 2–3) | Nine Seats (choose the Alliance or stay independent) · Ironroot Blood (clan adoption) · The Canyon Toll (Gale Canyons) |
| 14 Sunscar (Sage 3–SS1) | Glass and Bone (cross the desert) · The Sealed Gate · The Tomb King (seal taken — or kept from the Pilgrim) |
| 15 Pirates of the Starsea (SS1–2) | Gu's Ledger · The Skyport Wreck · Sect War (defence of the Alliance Gate) |
| 16 The Presence Trial (SS3) | Lu's Last Page · The Presence Trial · Stars Beyond (the Starsea launch, Act III hook) |

## Build phases

- **A · Foundation (done).** Zone-aware map; the crossing from the Ascension Gate; Cloudgate Port and the
  Thunderhorn Plains; Spark Weasel and Thunderhorn Rhino; Storm Ward jades and shards; chapter 11.
- **B · Heights and lake (done).** Rimefrost Heights, Mirrorwater Lake, four monsters, the Thousand-Eye Toad;
  chapter 12.
- **C · Nine Peaks (done).** Alliance or independence, auction house, clans (Ironroot), Gale Canyons;
  chapter 13. The Trial Hall moves to phase E with the Presence Trial.
- **D · Sunscar.** Desert, the Tomb of Sunscar and its King; chapter 14.
- **E · Starsea.** Skyport Wreck, pirates, star charts and vessels, sect war; chapters 15–16 and the
  Presence Trial.

## Attunement numbers

Four jades per zone; raising a jade from level n to n + 1 costs n + 1 of the zone's shards, up to 15, so
one jade costs 120 shards and a full set 480 (Storm Ward 60, the Skyport Wreck's need). Rooms ask for
their region's value unless they set their own (the Stormgrass Verge asks 6, the Lightning Scar 12);
safe rooms ask nothing. Spark Weasels drop a shard about one kill in three, Thunderhorn Rhinos one or two
in half their kills, and jars and chests of the Expanse carry them too.


## Pacing

The Part 4 table puts Sage at 70 h and Sage Sovereign at 110 h. Sage-level income is far above the
100 QP a minute the realm table's minutes assume (Sage Qi, denser Qi, better methods), so the Sage
stages need 1,050 target minutes per Level. Act II main quests pay 8% of the stage's need and side
quests 4% (the `act2_main` and `act2_side` buckets), because its stages are long and hold more
story. The balance simulator plays on to Sage Sovereign 1 and checks both targets (it lands Sage 1
at about 63 h and Sage Sovereign 1 at about 104 h, inside ±15%).

## Phase C · Nine Peaks, Ironroot and the Gale Canyons (built)

- **Travel.** A second sky-ship route from the Skydock to the Alliance Gate (chapter 13 onward), and a
  teleport stone at the Alliance Gate.
- **Nine Peaks (town).** Alliance Gate, Hall of Nine (the council and the path choice), Presence
  Terrace (Alliance sparring ladder), Auction Pavilion (the NPC auction house), Trial Hall (weekly
  trials later).
- **Alliance or independent (S20).** "Nine Seats" ends in a choice kept on the character: the Alliance
  (an envoy rank, Alliance missions, the Factor's discount, safe passage on every sky road) or the free
  path (Broker Mu's network, black-market lots, a lower auction fee, no toll). Either can be changed once
  at a price.
- **NPC auction house (S21, Sage 1, account slot 10).** Rotating lots of rare goods; each lot runs on the
  clock and NPC bidders raise against you by their own budgets (named Rng stream). Winning pays Spirit
  Stones; outbid stones return by mail.
- **Clans (Sage 2).** Ironroot Clan Hold (Hold Gate, Clan Hearth, Ancestor Hall): an adoption quest
  ("Ironroot Blood") makes you a clan member with a clan shop and a small body-growth bonus.
- **Gale Canyons (73–78, Storm Ward 40).** Canyon Mouth, Kite Winds, Harpy Roosts, Windbridge; Wind Kite
  and Canyon Harpy; wind-gust hazards on the bridges.
- **Chapter 13:** Nine Seats · Ironroot Blood · The Canyon Toll.

