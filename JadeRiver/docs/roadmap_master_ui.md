...
| 10 | **P7 · Wikis and volume**, **P10 · World plan and terminology**, **P8a · Beast Bands design** | Data and document work that can run in parallel with P5–P9 and must finish before v1.3's content is authored |
| 11 | v1.3 Star Frontier, with **P8b · Beast Bands** built inside it | A new zone is where a new headline system unlocks best; its beasts get band ranks from the start and the earlier zones' `beast_rank` values are already in the data |
...
| C5 | Master 8 asks for the Douluo Dalu soul-ring system; the rule is never to copy another work's names or content | Adapt the mechanics under an original name (working name Beast Bands), with Jade River's own colours per rank, arts and lore; the research page cites the source, the game never does. The user picks the final name (P8) |
...
1. **Realm names** (C1): keep Jade River's ladder with a Codex cross-reference, or rename? The recommendation is keep.
2. **The soul-ring adaptation's name** (C5): Beast Bands is a working name; the user picks.
3. **When the UI restyle starts** (§4): after Act III (recommended) or now, before v1.2 Phase C?
4. **The mockup approval** (C15): who approves, and in what form (PNG set under `docs/mockups/`, or a page)?
5. **The item target** (C12): named gear per archetype and zone, or a raw count?
6. **Beast Bands' unlock**: Spirit Awakening 1 with the Soul bar (recommended), or earlier at Qi Unfurling 7 with taming?
7. **Where the Full Review lives**: one page (`docs/review-v12.md`, growing through P11) or one page per section?
