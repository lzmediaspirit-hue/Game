...
def flame(cv, base, height, width, phase, mats, lean=0.0, name="flame", tongues=3):
    ...
    def shape(h, w, k):
        pts = []
        steps = 7
        # left side, bottom -> tip, with tongues
        for s in range(steps + 1):
            t = s / steps
            y = by - h * t
            half = w * (math.sin(math.pi * min(1.0, 0.25 + t * 0.9)) * (1 - t) ** 0.6)
            wob = 0.6 * math.sin(phase * 2.1 + t * 6.0 + k) * t
            pts.append((bx - half + lean * h * t * t + wob, y))
        tip = (bx + lean * h + 0.4 * math.sin(phase * 1.7 + k), by - h - 1.0 - 1.2 * (0.5 + 0.5 * math.sin(phase * 2.3 + k)))
        right = []
        for s in range(steps, -1, -1):
            ...
            if tongues and 0.3 < t < 0.8:
                tongue = 1.1 * max(0.0, math.sin(phase * 2.7 + t * 9.0 + k * 1.3))
            wob = 0.6 * math.sin(phase * 2.1 + t * 6.0 + k + 1.0) * t
            right.append((bx + half + tongue + lean * h * t * t + wob, y))
        return pts + [tip] + right

    outer, mid, core = (m if isinstance(m, px.Material) else px.flat(m) for m in mats)
    for (fh, fw, mat, k) in ((1.0, 1.0, outer, 0.0), (0.68, 0.6, mid, 1.3), (0.4, 0.3, core, 2.1)):
