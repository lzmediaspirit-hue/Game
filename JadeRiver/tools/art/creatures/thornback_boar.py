"""Thornback boar - elite: a hulking wild boar overgrown with thorny wood-green vines (Wood).

View: side, facing right.  ~37 art px tall to the thorn tips, ~58 long (cell 192).
Parts, back to front: far legs (dark), tufted tail, near hind leg, body (rump + shoulder hump
+ head + long snout shaded as one form), bristle shading, thorn spikes along the spine,
vines winding over the back and flank (with leaves), snout disc, long upcurved tusk, laid-back
ear, small fierce red eye under a heavy brow, near foreleg (pawing in the windup).
FX: snorting steam (windup), dust (paw, charge), impact.  Thorns bristle up when enraged.
"""
import math
...
HIDE = px.material("tb_hide", "#a57d59", "#735442", "#4c3a32", "#2f2426", outline="#110b0c",
                   thresholds=(0.9, 0.55, 0.18))
MANE = px.material("tb_mane", "#5a4436", "#3f2f29", "#2d2222", "#1e1719", outline="#0d0909")
VINE = px.material("tb_vine", "#a9dc6a", "#62a046", "#3a6c3c", "#23443a", outline="#0a1810")
LEAF = px.material("tb_leaf", "#c9f08a", "#7fbf52", "#4a8a44", "#2b5a3c", outline="#0a1810")
THORN = px.material("tb_thorn", "#f2e8b0", "#c9b878", "#8a7e4e", "#565034", outline="#141209")
SNOUT = px.material("tb_snout", "#d49a88", "#a86e62", "#7a4a48", "#4e3036", outline="#1a0c0e")
HOOF = px.material("tb_hoof", "#5e4a44", "#3e302e", "#2c2222", "#1c1617", outline="#0b0808")
STEAM = px.material("tb_steam", "#ffffff", "#dfe8e8", "#b2c2c6", "#8494a0", outline="#3a4650")
TUSK = px.BONE
EYE_RED = px.rgb("#ff5a3c")
EYE_DARK = px.rgb("#a8201e")
...
def _thorns(cv, spine, boost, slump):
    """Thorn spikes rising from the spine, leaning back, with dark bases."""
    spikes = ((0.12, 3.6), (0.24, 5.0), (0.36, 4.2), (0.5, 6.0), (0.62, 4.6), (0.74, 5.4), (0.86, 3.8))
    n = len(spine) - 1
    for t, h in spikes:
        f = t * n
        i = min(n - 1, int(f))
        q = px.lerp_pt(spine[i], spine[i + 1], f - i)
        ln = max(1.5, h + boost * (0.6 + 0.4 * (h / 6.0)))
        lean = 20 if not slump else 45
        tip = px.polar((q[0], q[1] + 1.5), 90 + lean, ln + 1.5)
        base_l = (q[0] - 1.7, q[1] + 1.8)
        base_r = (q[0] + 1.7, q[1] + 1.6)
        cv.polygon([base_l, tip, base_r], THORN, shade="soft", name="thorn")


def _vines(cv, C, spine, phase):
    """Thorny vines: one winding along the spine, two wrapping the shoulder and haunch.
    Drawn as real ropes (own shading + separation line), clipped to the body so they wrap."""
    pts = []
    for i, q in enumerate(spine):
        pts.append((q[0], q[1] + 2.4 + 1.6 * math.sin(i * 1.9 + phase)))
    pts = [px.lerp_pt(a, b, t) for a, b in zip(pts, pts[1:]) for t in (0.0, 0.5)] + [pts[-1]]
    cv.limb(pts, 1.3, VINE, shade="soft", name="vine", sep="deep", clip="body")
    for (x0, y0), (xm, ym), (x1, y1) in (((9.0, -11.5), (12.5, -3.0), (10.5, 5.5)),
                                         ((-11.0, -8.5), (-7.0, -1.0), (-10.0, 7.0))):
        cv.limb([(C[0] + x0, C[1] + y0), (C[0] + xm, C[1] + ym), (C[0] + x1, C[1] + y1)], 1.15, VINE, shade="soft",
                name="vine", sep="deep", clip="body")
    # small thorns pricking off the vines
    for (x, y) in ((-6.0, -9.5), (1.0, -10.8), (12.6, -5.0), (-7.2, -3.0), (11.8, 2.0)):
        cv.pixel(math.floor(C[0] + x), math.floor(C[1] + y), THORN.light, name="vine")
    # leaves sprouting off the vine
    for (x, y, a) in ((-12.0, -8.5, 160), (-2.0, -12.5, 125), (6.5, -12.2, 55), (13.0, -2.0, 20)):
        root = (C[0] + x, C[1] + y)
        mid = px.polar(root, a, 1.8)
        tip = px.polar(mid, a + 25, 2.2)
        cv.limb([root, mid, tip], [0.6, 1.3, 0.4], LEAF, shade="soft", name="leaf", sep="deep")
...
def draw(cv: px.Canvas, action: str, frame: int) -> None:
    ...
    with cv.xform(*xf):
        ...
        with cv.xform(px.rotate(p.head, neck)):
            # snout disc + nostrils
            cv.ellipse(snout_end[0] + 0.3, snout_end[1] - 0.2, 1.8, 3.4, SNOUT, name="snout", sep="deep")
            cv.pixels([(math.floor(snout_end[0]), math.floor(snout_end[1]) - 2),
                       (math.floor(snout_end[0]), math.floor(snout_end[1]) + 1)], SNOUT.deep, name="nostril")
            # mouth line + long upcurved tusk
            cv.line((math.floor(H[0] + 4), math.floor(H[1] + 5)), (math.floor(H[0] + 9), math.floor(H[1] + 6)),
                    HIDE.deep, name="mouth")
            tusk = [(H[0] + 7.0, H[1] + 6.2), (H[0] + 10.6, H[1] + 4.8), (H[0] + 12.0, H[1] + 1.0),
                    (H[0] + 11.0, H[1] - 2.4)]
            cv.limb(tusk, [1.4, 1.2, 0.9, 0.5], TUSK, shade="soft", name="tusk", sep="deep")
            # laid-back ear
            ep = (H[0] - 3.0, H[1] - 5.0)
    ...
    if p.hit:
        ...
        px.impact(top, hit_at[0], hit_at[1], size=4)
