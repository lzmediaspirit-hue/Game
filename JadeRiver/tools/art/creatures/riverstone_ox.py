"""Riverstone ox - a mount-only spirit ox (S46): a broad, patient ox with a river-grey hide,
smooth river pebbles grown into its back, moss in its mane and two heavy horns curving out
and up.  Built on the Thunderhorn rhino rig (cell 192); no lightning - it snorts and charges.
"""
import math
...
HIDE = px.material("ro_hide", "#d4d2c8", "#aaa89c", "#7c7a70", "#54534c", outline="#161614",
                   thresholds=(0.9, 0.55, 0.18))
MANE = px.material("ro_moss", "#a4c47c", "#78985a", "#56743e", "#3a522a", outline="#10180c")
PLATE = px.material("ro_pebble", "#e2e6e2", "#aab6b4", "#76868a", "#4e5c62", outline="#121416")
HORN = px.material("ro_horn", "#f4ecd8", "#cfc2a0", "#9a8c6c", "#665a44", outline="#1a140c")
SNOUT = px.material("ro_muzzle", "#c8b8a8", "#a08e7e", "#76665a", "#50443c", outline="#121010")
...
def draw(cv: px.Canvas, action: str, frame: int) -> None:
    ...
    with cv.xform(*xf):
        ...
        with cv.xform(px.rotate(p.head, neck)):
            ...
            # two heavy horns sweeping forward and up from the crown (the near one drawn over the far)
            horn2 = [(H[0] - 1.0, H[1] - 5.0), (H[0] + 3.5, H[1] - 8.0), (H[0] + 5.0, H[1] - 12.0)]
            cv.limb(horn2, [1.8, 1.3, 0.5], HORN.step(1), shade="soft", name="tusk", sep="deep")
            horn = [(H[0] + 1.0, H[1] - 4.6), (H[0] + 6.0, H[1] - 6.0), (H[0] + 9.0, H[1] - 9.5), (H[0] + 9.0, H[1] - 13.0)]
            cv.limb(horn, [2.4, 1.9, 1.2, 0.5], HORN, shade="soft", name="tusk", sep="deep")
            tusk = horn
            # laid-back ear
            ep = (H[0] - 3.0, H[1] - 5.0)
