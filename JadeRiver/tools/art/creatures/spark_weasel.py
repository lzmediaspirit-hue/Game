"""Spark weasel - a long, low golden weasel whose tail tip crackles with lightning (Thunder).

Azure Expanse, Thunderhorn Plains (64-66).  Derived from the ember fox rig: the body is
stretched long and carried low, the ears are small and round, the bushy tail is slim and
ends in a white-hot spark that throws bolts.  Windup: arches and the tail charges with
arcs; attack: a darting bite that discharges a bolt; death: curls up and fades.
"""
import math
...
FUR = px.material("weasel_fur", "#fff0a0", "#e8bd3c", "#a8792a", "#5e4424", outline="#1a1208",
                  thresholds=(0.92, 0.58, 0.22))
WHITE = px.material("weasel_white", "#fffdf0", "#f1ead0", "#c8bea0", "#8c8266", outline="#1a1208")
SOCK = px.material("weasel_sock", "#4e5b78", "#34405a", "#252e44", "#18202f", outline="#0b0f18")
EAR_IN = px.material("weasel_ear_in", "#dfe8ff", "#aebce0", "#7f8db4", "#56608a", outline="#1a1208")
FLAME = (px.rgb("#5b8cff"), px.rgb("#a8d4ff"), px.rgb("#f4fbff"))   # the charged tail spark
IRIS = px.rgb("#7fd4ff")
...
DEFAULTS = dict(bx=0, by=0, crouch=0, pitch=0, head=0, jaw=0.0, ear=0,
                ...
                sparks=None, hit=False, curl=False, fade=1.0, breathe=0.0, stretch=1.55)
...
POSE = px.poses(DEFAULTS, {
    ...
    "attack": [  # pounce and ember bite; the jaws snap on frame 1 with a burst of sparks
        dict(bx=4, by=-4, pitch=10, head=6, jaw=1.0, ear=-30, eye="angry", tail=(10, 2.0), flame=11.0,
             ph=4.8, feet=((4, 5), (3, 4), (-3, 1), (-4, 0)), stretch=1.7),
        dict(bx=5, by=-1, pitch=-4, head=-6, jaw=0.1, ear=-25, eye="angry", tail=(6, 3.0), flame=9.5,
             ph=6.0, feet=((3, 0), (2, 0), (-3, 1), (-4, 1)), sparks=0.25, hit=True, stretch=1.62),
        dict(bx=5, pitch=-2, head=-3, jaw=0.2, ear=-15, eye="angry", tail=(4, 4.0), flame=8.5, ph=7.2,
...
def _tail(cv, root, p, big=1.0):
    ...
    for k, (ln, a) in enumerate(segs):
        ...
        pts.append(q)
    cv.limb(pts, [1.3, 1.6, 1.5, 1.1], FUR, name="tail")
    tip = pts[-1]
    back = pts[-2]
    cv.limb([px.lerp_pt(back, tip, 0.6), tip], [1.5, 1.1], SOCK, shade="two", decal=True, clip="tail")
    d = math.degrees(math.atan2(-(tip[1] - back[1]), tip[0] - back[0]))
    lean = math.cos(math.radians(d)) * 0.35 - 0.15
    base = px.polar(tip, d, 1.0)
    # a small white-hot spark that grows while charging
    hc.flame(cv, base, p.flame * 0.55, 1.4 + p.flame * 0.06, p.ph, FLAME, lean=lean, name="flame", tongues=2)
    return tip
...
def _ear(cv, H, ang, far):
    ...
    pts = [px.rot_pt(q, ang, pivot) for q in ((H[0] - 2.6, H[1] - 2.2), (H[0] - 1.6, H[1] - 5.6),
                                              (H[0] + 0.6, H[1] - 3.0))]
    if far:
...
def draw(cv: px.Canvas, action: str, frame: int) -> None:
    ...
    ground = cv.ground
    cr = p.crouch
    C = (cv.gx - 2 + p.bx, ground - 8.6 + p.by + cr)
    br = p.breathe
    ...
    with cv.xform(*body_x):
        body_g = [cv.geom_limb([(R[0] - 0.8, R[1] + 0.2), (C[0], C[1] - 0.9 - br * 0.5), (K[0] + 0.4, K[1])],
                               [3.3 + br * 0.4, 3.0 + br, 2.9 + br * 0.4])]
        with cv.xform(px.rotate(p.head, N0)):
            ...
            head_g = [cv.geom_limb([N0, H], [2.6, 2.4]), cv.geom_ellipse(H[0], H[1], 3.3, 2.9),
                      cv.geom_limb([(H[0] + 1.4, H[1] + 0.6), (H[0] + 5.4, H[1] + 1.5)], [2.1, 1.0])]
        cv.draw_geom(cv.union(*body_g, *head_g, weights=[1.0, 0.6, 0.9, 0.75]), FUR, name="body", sep="deep")
        # a dark blue stripe down the spine, the weasel's mark
        cv.limb([(R[0] - 1.0, R[1] - 2.8), (C[0], C[1] - 3.7 - br * 0.5), (K[0] + 0.5, K[1] - 2.6)], [0.9, 1.0, 0.8],
                SOCK, shade="two", decal=True, clip="body")
        ...
        with cv.xform(px.rotate(p.head, N0)):
            ...
            cv.limb([(H[0] - 2.2, H[1] - 1.4), (H[0] + 2.4, H[1] - 0.6)], [1.2, 0.9], SOCK, shade="two", decal=True, clip="body")
            _eye(cv, round(H[0] + 0.2), round(H[1] - 1.2), p.eye)
            jaws = cv.tp((H[0] + 6.4, H[1] + 2.0))
    ...
    if p.sparks is not None:
        ...
        hc.sparks(fx, jaws[0] + 1, jaws[1], p.sparks, n=6, radius=7, seed=3, aim=40, spread=200,
                  colors=(BOLT_CORE, BOLT_EDGE))
    if action == "windup" and frame >= 1:
        # arcs crawl along the raised tail while it charges
        fx2 = cv.layer(above=True, outline=False)
        tail_tip = cv.tp((C[0] - 10.0, C[1] - 9.0 - frame * 1.5))
        hc.bolt(fx2, tail_tip, (tail_tip[0] + 3.0, tail_tip[1] - 5.0), seed=frame, jag=1.2, segs=4,
                core=BOLT_CORE, edge=BOLT_EDGE, fork=frame == 2)
    if p.hit:
        top = cv.layer(above=True, outline=False)
        hc.bolt(top, (jaws[0] - 1, jaws[1] - 4), (jaws[0] + 6, jaws[1] + 1), seed=7, jag=1.5, segs=5,
                core=BOLT_CORE, edge=BOLT_EDGE)
        px.impact(top, jaws[0] + 1, jaws[1], size=3, color=px.rgb("#a8d4ff"))
