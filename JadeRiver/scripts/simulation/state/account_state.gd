...
var works: Dictionary = {}             # S50 V10d the account web: seals, steles, favours, ... (Post authority)
...
func restore(d: Dictionary) -> void:
	...
	works = (d.get("works", {}) as Dictionary).duplicate(true) if d.get("works", {}) is Dictionary else {}
	leaves = {}
	var lv = d.get("leaves", {})
	if lv is Dictionary:
		for k in lv:
			if int(lv[k]) > 0: leaves[str(k)] = int(lv[k])
