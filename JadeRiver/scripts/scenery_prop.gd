...
## Blocking scenery (trees, rocks, stalls): art from the painted prop atlas
## (`cell`) or from data/prop_art.json (`prop`). Collision is the footprint data.
var spec: Dictionary
var t:=0.0
func _ready():
	...
	if spec.has("prop"):
		var st: Dictionary=SpriteCache.prop(str(spec.prop)).get("states",{}).get(str(spec.get("state","idle")),{})
		set_process(int(st.get("frames",1))>1)
	else:
		set_process(false)
	queue_redraw()
func _process(delta):
...
func _draw():
	if spec.has("prop"):
		SpriteCache.draw_prop(self,str(spec.prop),str(spec.get("state","idle")),t,Vector2.ZERO,bool(spec.get("flip",false)))
		return
