...
func draw_page() -> void:
	...
	btn(Rect2(content.position.x, content.end.y - 58, 210, 54), "Claim all", "claim_all", null, true)
	...
	text(right.position + Vector2(24, 74), "From %s" % str(mm.from), 17, UiKit.MIST)
	...
	if not (mm.get("attachments", []) as Array).is_empty() and not mm.get("claimed", false):
		btn(Rect2(right.end.x - 224, right.end.y - 70, 200, 54), "Claim", "claim", int(mm.id), true)
	btn(Rect2(right.position.x + 24, right.end.y - 70, 160, 54), "Delete", "delete", int(mm.id))
