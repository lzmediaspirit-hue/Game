...
func draw_page() -> void:
	...
	panel(left)
	panel(right)
	heading(left.position + Vector2(20, 40), "Sect missions", left.size.x - 40)
	var y := left.position.y + 60
	if ch.quests.daily.is_empty():
		para(Rect2(left.position.x + 20, y, left.size.x - 40, 100), "Missions open at Bone Forging 6 for sect disciples. They refresh at dawn." if not Unlocks.is_unlocked(ch.id, "daily_missions") else "All of today's missions are done. New ones at dawn.", 18, UiKit.MIST)
	for q in ch.quests.daily:
		...
		y += 60
	heading(right.position + Vector2(20, 40), "Requests", right.size.x - 40)
	...
	for qid in ch.quests.offered:
		...
		text(Vector2(right.position.x + 20, yy + 46), "Ask %s" % ContentDB.name_of("npcs", str(d2.giver)), 16, UiKit.MIST)
		yy += 60
