...
func draw_page() -> void:
	...
	panel(r)
	if str(ts.get("id", "")) == "":
		para(Rect2(r.position + Vector2(30, 30), r.size - Vector2(60, 60)), "You are Unaffiliated. The Jade Sect and the Cloud Sect recruit at the Stoneford Fairground.", 22, UiKit.PAPER)
		return
	...
	text(r.position + Vector2(30, 90), "Rank: %s" % rank_name, 22)
	...
	for j in order.size():
		...
		if j == i + 1:
			...
			text(Vector2(r.position.x + 300, y), RequirementRules.first_failure_text(rk.get("requires", {}), Game.ctx(ch)) if not ok else "Ready", 17, UiKit.PAPER if ok else UiKit.MIST)
			btn(Rect2(r.position.x + 640, y - 30, 220, 46), "Promotion trial", "promote", null, true, ok, "Not yet")
		y += 40
	btn(Rect2(r.end.x - 260, r.end.y - 76, 230, 56), "Sect shop", "shop", null, false, Unlocks.is_unlocked(ch.id, "contribution_shop"), Unlocks.locked_text("contribution_shop"))
	btn(Rect2(r.end.x - 520, r.end.y - 76, 230, 56), "Missions", "missions")
