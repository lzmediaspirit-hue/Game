...
func setup() -> void:
	...
	tabs = [{"id": "overview", "label": "Overview"},
		{"id": "foundation", "label": "Foundation", "locked": "" if Unlocks.is_unlocked(ch.id, "foundation") else Unlocks.locked_text("foundation")},
		{"id": "methods", "label": "Methods"},
		{"id": "dao", "label": "Dao", "locked": "" if Unlocks.is_unlocked(ch.id, "dao_tree") else Unlocks.locked_text("dao_tree")},
		{"id": "seclusion", "label": "Seclusion", "locked": "" if Unlocks.is_unlocked(ch.id, "seclusion") else Unlocks.locked_text("seclusion")}]
...
func _overview(ch) -> void:
	...
	text(Vector2(x + 110, y + 70), "Level %d · %s" % [ProgressionRules.level(ch), str(cu.state).capitalize()], 19, UiKit.MIST)
	y += 120
	...
	var rows := [["Method", ContentDB.name_of("methods", cu.method_id) if cu.method_id != "" else "None"],
		...
		["Stability", str(cu.stability).capitalize()],
		["Body Level", "%d (%d%%)" % [cu.body_level, int(100.0 * cu.body_xp / maxf(1.0, ProgressionRules.body_xp_needed(cu.body_level)))]],
		["Toxicity", "%d / %d" % [int(cu.toxicity), int(ch.stats.value("toxicity_tolerance"))]],
	...
	btn(Rect2(x + 240, left.end.y - 78, 230, 58), "Breakthrough", "breakthrough", null, cu.state == "bottleneck",
	...
	heading(right.position + Vector2(24, 40), "Next: %s" % ContentDB.name_of("realms", str(q.get("to", ""))), right.size.x - 48)
	var yy := right.position.y + 70
	if not q.get("major", false):
		para(Rect2(right.position.x + 24, yy, right.size.x - 48, 120), "A minor step. Fill the bar and break through: no requirements, no risk.", 19, UiKit.PAPER)
	else:
		for r in q.get("results", []):
			...
			text(Vector2(right.position.x + 56, yy + 40), ("Required" if r.hard else "Lowers risk") + " · " + str(r.cause).capitalize(), 14, UiKit.MIST)
			yy += 52
		text(Vector2(right.position.x + 24, right.end.y - 40), "Risk: %s · Success %d%%" % [str(q.risk).capitalize(), int(float(q.success) * 100)], 20, UiKit.GOLD)

func _foundation(ch) -> void:
	var cu: CultivatorState = ch.cultivator
	var r := Rect2(content.position.x, content.position.y, content.size.x, content.size.y)
	panel(r)
	text(r.position + Vector2(24, 40), "Unspent meridian points: %d" % cu.unspent_meridian_points, 22, UiKit.PALE_GOLD)
	var names := {"body": "Body", "agility": "Agility", "essence": "Essence", "spirit": "Spirit", "insight": "Insight"}
	var desc := {"body": "HP, defence, body training", "agility": "Speed, evasion, accuracy", "essence": "QI, Qi attack", "spirit": "Soul, sense, will",
		"insight": "Insight, mastery, crafting"}
	...
	for k in names:
		...
		btn(Rect2(r.position.x + 850, y + 2, 120, 44), "+1", "meridian", k, true, cu.unspent_meridian_points > 0, "No points to spend")
		y += 64
	var free := not ProgressionRules.at_least(cu.realm_key, "qi_unfurling_1")
	btn(Rect2(r.position.x + 24, r.end.y - 70, 300, 52), "Reset (free)" if free else "Reset (Meridian Reversal Pill)", "reset_meridians")
...
func _methods(ch) -> void:
	...
	if cu.methods_known.is_empty():
		para(Rect2(r.position + Vector2(30, 30), r.size - Vector2(60, 60)), "You know no cultivation method yet. Lu will teach you one.", 21, UiKit.MIST)
		return
	list("methods", r.grow(-14), cu.methods_known.size(), 110, func(i: int, rr: Rect2):
		...
		text(rr.position + Vector2(20, 64), "%s · %s affinity · rate ×%.2f · capacity ×%.2f · ceiling %s" % [str(m.get("grade", "")).capitalize(),
		...
		text(rr.position + Vector2(20, 90), "Compatibility: %s" % ProgressionRules.method_compatibility(ch, mid).capitalize(), 16, UiKit.BRIGHT_JADE)
		if not active: btn(Rect2(rr.end.x - 170, rr.position.y + 26, 150, 50), "Switch", "switch", mid)
		else: text(Vector2(rr.end.x - 170, rr.position.y + 58), "Active", 20, UiKit.GOLD, HORIZONTAL_ALIGNMENT_CENTER, 150)
	)
...
func _dao(ch) -> void:
	...
	if ids.is_empty():
		para(Rect2(r.position + Vector2(30, 30), r.size - Vector2(60, 60)), "No Dao insight yet. Use techniques, meditate at insight stones and craft to begin comprehending.", 21, UiKit.MIST)
		return
	var tiers := ["Unaware", "Observation", "Imitation", "Reliable Execution", "Explanation", "Adaptation", "Original Application"]
	list("daos", r.grow(-14), ids.size(), 76, func(i: int, rr: Rect2):
		...
		bar(Rect2(rr.position.x + 330, rr.position.y + 18, 520, 30), ins / maxf(1.0, need), UiKit.SOUL, "%s / %s insight" % [UiKit.fmt(ins), UiKit.fmt(need)])
		if Unlocks.is_unlocked(ch.id, "contemplate"):
			btn(Rect2(rr.end.x - 170, rr.position.y + 12, 150, 44), "Contemplate", "contemplate", d)
	)
...
func _seclusion(ch) -> void:
	...
	if cur != "":
		text(Vector2(r.position.x + 24, r.end.y - 30), "Set: %s. Close the game and your disciple cultivates." % cur.replace("_", " ").capitalize(), 19, UiKit.BRIGHT_JADE)
...
func on_action(id: String, data) -> void:
	...
	match id:
		...
		"reset_meridians": ask("Reset all meridian points?", "reset_yes")
		"reset_yes": submit({"type": "reset_meridians"})
		"switch":
			var cost := Game.progression.method_switch_preview(ch, false)
			ask("Switch method? You lose %s QP and your foundation becomes Unstable for a while." % UiKit.fmt(cost), "switch_yes", data)
		"switch_yes": submit({"type": "switch_method", "id": str(data)})
		"contemplate":
			submit({"type": "set_contemplate", "dao": str(data)})
			flash("Contemplating %s in seclusion" % ContentDB.name_of("daos", str(data)))
		"focus":
			if submit({"type": "enter_seclusion", "focus": str(data)}).get("ok", false): flash("Seclusion set.")
