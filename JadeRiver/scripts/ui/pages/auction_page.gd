...
func draw_page() -> void:
	...
	text(r.position + Vector2(260, 44), Tx.t("ui.auction.premium") % int(round(Game.economy.auction_fee(ch, house) * 100.0)), 18, UiKit.MIST)
	var lots: Array = Game.economy.auction_lots(house).filter(func(l): return not l.get("closed", false))
	if lots.is_empty():
