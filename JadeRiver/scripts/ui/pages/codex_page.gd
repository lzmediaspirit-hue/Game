...
func _collection() -> void:
	...
	list("col", r.grow(-10), rows, 150, func(row: int, rr: Rect2):
		for col in cols:
			...
			if kills > 0:
				var tex := SpriteCache.creature(str(e.get("art", {}).get("creature", e.id)) if e.get("art") is Dictionary else str(e.id))
				text(cr.position + Vector2(0, 100), str(e.name), 16, UiKit.PAPER, HORIZONTAL_ALIGNMENT_CENTER, cr.size.x)
				text(cr.position + Vector2(0, 124), "Defeated %d" % kills, 14, UiKit.MIST, HORIZONTAL_ALIGNMENT_CENTER, cr.size.x)
