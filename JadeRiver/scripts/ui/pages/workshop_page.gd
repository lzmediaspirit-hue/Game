...
const TABS := [["formations", "Formations", "formations"], ["appraisal", "Appraisal", "appraisal"], ["healing", "Infirmary", "healing"],
	["puppetry", "Puppets", "puppetry"], ["research", "Research", "research"], ["teaching", "Teaching", "teaching"]]
...
func _formations(ch) -> void:
	...
	panel(left)
	heading(left.position + Vector2(20, 44), "Blueprints", 520)
	...
	for bp in rows:
		...
		text(r.position + Vector2(16, 108), "%d nodes · %d %s" % [int(bp.nodes), need, ContentDB.item_name(str(bp.fuel))], 15, UiKit.PALE_GOLD)
		btn(Rect2(r.end.x - 160, r.position.y + 34, 144, 50), "Place", "place", str(bp.id), true, ok, Unlocks.locked_text(str(bp.get("unlock", "formations"))))
		y += 128
	...
	panel(right)
	heading(right.position + Vector2(20, 44), "Standing formations", right.size.x - 40)
	var live: Array = Game.workshop.active_formations(ch)
	if live.is_empty():
		para(Rect2(right.position + Vector2(24, 70), right.size - Vector2(48, 90)), "None. Stand where you cultivate or fight, then place one. Fuel burns even while you are away.", 18, UiKit.MIST)
	...
	for i in live.size():
		...
		text(r2.position + Vector2(14, 56), "%s · %.1f h of fuel" % [ContentDB.room(str(f.room)).get("name", f.room), hours], 15, UiKit.MIST)
		btn(Rect2(r2.end.x - 120, r2.position.y + 12, 106, 46), "Dispel", "dispel", i)
		yy += 80

func _appraisal(ch) -> void:
	var r := Rect2(content.position, content.size)
	panel(r)
	heading(r.position + Vector2(20, 44), "Unknown goods", r.size.x - 40)
	var has_tool: bool = Game.crafting.tool_power(ch, "appraisal") > 0.0
	para(Rect2(r.position + Vector2(24, 60), Vector2(r.size.x - 48, 60)),
		"Look closely: real jade is cold and heavy, glass is warm. " + ("Your loupe is ready." if has_tool else "You need an Appraiser's Loupe (Elder Gu's Trade House)."), 18, UiKit.MIST)
	var items: Array = []
	...
	if items.is_empty():
		para(Rect2(r.position + Vector2(24, 140), Vector2(r.size.x - 48, 60)), "Nothing to appraise. Curios turn up in trade houses and old chests.", 18, UiKit.MIST)
	...
	for idx in items:
		...
		btn(Rect2(x - 6, r.position.y + 234, 96, 46), "Appraise", "appraise", idx, true, has_tool, "Needs a loupe")
		x += 110
...
func _healing(ch) -> void:
	var r := Rect2(content.position, content.size)
	panel(r)
	heading(r.position + Vector2(20, 44), "The infirmary", r.size.x - 40)
	...
	para(Rect2(r.position + Vector2(24, 64), Vector2(r.size.x - 48, 90)),
		"Guide your Qi through the needles to clear a patient's blockage. Each treatment costs %d%% of your Qi and earns %d contribution. Patients waiting today: %d." % [int(float(p.get("qi_cost_pct", 0.15)) * 100), int(p.get("contribution", 20)), maxi(0, left)], 18)
	var here: bool = Game.workshop.npc_here(ch, p.get("npcs", []))
	btn(Rect2(r.position.x + 24, r.position.y + 170, 260, 56), "Treat a patient", "treat", null, true, here and left > 0, "Patients wait in the sect infirmary" if not here else "No more patients today")
...
func _puppets(ch) -> void:
	...
	for b in p.get("blueprints", []):
		...
		text(r.position + Vector2(16, 88), "Gathers " + yl, 15, UiKit.MIST)
		btn(Rect2(r.end.x - 160, r.position.y + 40, 144, 50), "Build", "build", str(b.id), true, here, "Build at Tinkerer Yu's bench")
		y += 140
	var right := Rect2(left.end.x + 20, content.position.y, content.end.x - left.end.x - 20, content.size.y)
	panel(right)
	heading(right.position + Vector2(20, 44), "Your puppets", right.size.x - 40)
	...
	para(Rect2(right.position.x + 24, yy2 + 10, right.size.x - 48, 80), "Waiting: " + (summary if summary != "" else "nothing yet"), 17, UiKit.MIST)
	btn(Rect2(right.position.x + 24, right.end.y - 80, 220, 54), "Collect", "collect", null, true, not got.is_empty(), "Nothing gathered yet")
...
func _research(ch) -> void:
	...
	var r := Rect2(content.position, content.size)
	panel(r)
	heading(r.position + Vector2(20, 44), "Restore a damaged manual", r.size.x - 40)
	para(Rect2(r.position + Vector2(24, 64), Vector2(r.size.x - 48, 90)), "Water, worms and time eat manuals. Ink and insight give them back. What survives is never quite certain.", 18, UiKit.MIST)
	var x := r.position.x + 24
	for inp in p.get("inputs", []):
		slot_box(Rect2(x, r.position.y + 150, 84, 84), str(inp.item), ch.inventory.count(str(inp.item)))
		text(Vector2(x, r.position.y + 256), "Need %d" % int(inp.count), 15, UiKit.MIST)
		x += 110
	var here: bool = Game.workshop.npc_here(ch, p.get("npcs", []))
	btn(Rect2(r.position.x + 24, r.position.y + 290, 240, 56), "Restore", "restore", null, true, here, "Use the library's bench")
	_rank_line(ch, "research", Vector2(r.position.x + 16, r.end.y - 14))

func _teaching(ch) -> void:
	var r := Rect2(content.position, content.size)
	panel(r)
	heading(r.position + Vector2(20, 44), "Teach your sect's disciples", r.size.x - 40)
	var daos: Array = Game.workshop.teachable_daos(ch)
	if not Game.sect.founded():
		para(Rect2(r.position + Vector2(24, 64), Vector2(r.size.x - 48, 90)), "Found your own sect first. Its disciples are yours to teach.", 18, UiKit.MIST)
		return
	para(Rect2(r.position + Vector2(24, 64), Vector2(r.size.x - 48, 60)),
		("You can teach: " + ", ".join(daos.map(func(d): return ContentDB.name_of("daos", str(d))))) if not daos.is_empty() else "Reach Explanation in a Dao before you can teach it.", 18, UiKit.MIST)
	...
	list("teach", Rect2(r.position + Vector2(16, 130), Vector2(r.size.x - 32, r.size.y - 170)), ds.size(), 64, func(i: int, rr: Rect2):
		...
		text(rr.position + Vector2(16, 38), "%s · Lv %d" % [str(d.get("name", "Disciple")), int(d.get("level", 1))], 19)
		btn(Rect2(rr.end.x - 150, rr.position.y + 7, 136, 48), "Teach", "teach", i, true, not daos.is_empty(), "No Dao at Explanation")
	)
...
func on_action(id: String, data) -> void:
	match id:
		"place":
			...
			if r.get("ok", false): flash("Formation placed: %d h of fuel." % int(r.get("hours", 0)))
		...
		"build":
			if submit({"type": "build_puppet", "blueprint": str(data)}).get("ok", false): flash("The puppet stirs and sets to work.")
		...
		"teach":
			...
			if r5.get("ok", false): flash("Lesson given.")
	queue_redraw()
