...
func draw_page() -> void:
	...
	for s in stones:
		...
		btn(Rect2(content.position.x, y, content.size.x, 58), Tx.t("ui.teleport.shard") % [str(s.name) if known else Tx.t("ui.teleport.undiscovered"), int(s.get("fee_shards", 1))], "go", str(s.id), false, ok, why)
		y += 66
